...
def replace_with_Ns(probN, seq_i, seq, trim_N=True):
    ...
    OUT: returns the sequence where bases with no read support are
         replaced with "N", and terminal N's are trimmed
    """
...
def rename(wd=os.getcwd(), output_file="renamed.fasta", record_prefix='',
           prob_min=None, no_N=False, no_trim_N=True):
    """
    wd is an iteration directory
    prob_min is minimum prior prob of a sequence to include in output.
    If None, include all seqs.
    """
    ...
    sorted_records = []
    iterfile = file(os.path.join(wd, "iter.%02d.cons.fasta" % current_iter))
    for record in SeqIO.parse(iterfile, "fasta"):
    ...
    normed_priors = [prior / len(record.seq)
                     for prior, record in sorted_records]
    ...
    for i, (prior, record) in enumerate(sorted_records):
        record.description = "Prior=%06f Length=%d NormPrior=%06f" % (
...
def main(argv=None):
    ...
    parser.add_option(
        '-p', '--prob_min', type='float',
        help='Only include sequences in output with prior probability above ' +
             'PROB_MIN (Default: include all sequences)'
    )
    parser.add_option(
        '-r', '--record_prefix', type='string', default='',
        help='Add the specified prefix to each fasta record title'
    )
    parser.add_option(
        '-n', '--no_N', action="store_true",
        help="Don't change bases with no read support to N.  Caution: these " +
             "bases are not supported by reads in the input data, but will " +
             "usually be from a closely related sequence."
    )
    parser.add_option(
        '-t', '--no_trim_N', action="store_true",
        help="Don't trim off N bases with no read support from ends of " +
             "sequences.  Ignored if --no_N is also passed"
    )
    parser.add_option(
        '-o', '--output_file',
        type="string",
    ...
    if len(args) == 0:
        ...
        parser.error("Found more than one argument on command line (expects " +
                     "only a single iter.DIR plus any options): %s" % (args))
    if not os.path.exists(wd):
        parser.error("Directory not found: %s" % (wd))
    if re.match("iter.[0-9]+$", os.path.basename(wd)) is None:
        parser.error("Directory %s is not a valid emirge iter.DIR (try " +
                     "--help for full usage)" % (wd))

    rename(wd, options.output_file, options.prob_min, options.record_prefix,
           options.no_N, options.no_trim_N)
